...
{
    int r = randInt(1, NUMDIRS - 1);
    if (r < 3) //1, 2 == 66.666%
        return true;
    else // when r is 3 == 33.3333%
        return false;
}
...
{
    //if r matches the row of where player is located
    bool rowMatch = (r == m_player->row());
    //if c matches the column of where player is located
    bool colMatch = (c == m_player->col());

    //If both matches, it means that at row r and col c, there is a player.
    ...
    else
    {
        for (int k = 0; k < m_nFlatulans; k++)
        {
            if (m_flatulans[k] == nullptr)
                continue;
            if (r == m_flatulans[k]->row() && c == m_flatulans[k]->col())
                nOfFlatAt++;
        }
    }
    ...
    for (int k = 0; k < m_nFlatulans; k++)
    {
        ...
        for (int j = k + 1; j < m_nFlatulans; j++)
        {
            if (m_flatulans[j] == nullptr)
                continue;
            bool RowMatch = (m_flatulans[k]->row() - 1 == m_flatulans[j]->row() - 1);
            bool ColMatch = (m_flatulans[k]->col() - 1 == m_flatulans[j]->col() - 1);
            //if more than one Flatulan occupies the same position, increase the num by 1
            if (RowMatch && ColMatch)
                numOfFlatAtSamePos++;
        }
        ...
        if (numOfFlatAtSamePos == 0)
        {
            char& gridChar = grid[m_flatulans[k]->row() - 1][m_flatulans[k]->col() - 1];
            if (gridChar == '.' && !isdigit(gridChar))
                gridChar = 'F';
        }

        else if (numOfFlatAtSamePos < 9)
        {
            char& gridChar = grid[m_flatulans[k]->row() - 1][m_flatulans[k]->col() - 1];
...
void City::preachToFlatulansAroundPlayer()
{
    for (int k = 0; k < m_nFlatulans; k++)
    {
        if (m_flatulans[k] == nullptr)
            continue;
        //Located UP
        bool Up = (m_flatulans[k]->row() == m_player->row() - 1 && m_flatulans[k]->col() == m_player->col());
        //Located Down
        bool Down = (m_flatulans[k]->row() == m_player->row() + 1 && m_flatulans[k]->col() == m_player->col());
        //Located Left
        bool Left = (m_flatulans[k]->row() == m_player->row() && m_flatulans[k]->col() == m_player->col() - 1);
        //Located Right
        bool Right = (m_flatulans[k]->row() == m_player->row() && m_flatulans[k]->col() == m_player->col() + 1);
        //Located Up-Left
        bool UpLeft = (m_flatulans[k]->row() == m_player->row() - 1 && m_flatulans[k]->col() == m_player->col() - 1);
        //Located Up-Right
        bool UpRight = (m_flatulans[k]->row() == m_player->row() - 1 && m_flatulans[k]->col() == m_player->col() + 1);
        //Located Down-Left
        bool DownLeft = (m_flatulans[k]->row() == m_player->row() + 1 && m_flatulans[k]->col() == m_player->col() - 1);
        //Located Down-Right
        bool DownRight = (m_flatulans[k]->row() == m_player->row() + 1 && m_flatulans[k]->col() == m_player->col() + 1);

        bool checkIfAround = (Up || Down || Left || Right || UpLeft || UpRight || DownLeft || DownRight);
        if (checkIfAround && m_flatulans[k]->possiblyGetConverted())
        {
            delete m_flatulans[k];
...
void City::moveFlatulans()
{
    for (int k = 0; k < m_nFlatulans; k++)
    {
        ...
        //If Flatulan is orthogonally adjacent to the player
        //Located Up
        bool Up = (m_flatulans[k]->row() - 1 == m_player->row() && m_flatulans[k]->col() == m_player->col());
        //Located Down
        bool Down = (m_flatulans[k]->row() + 1 == m_player->row() && m_flatulans[k]->col() == m_player->col());
        //Located Left
        bool Left = (m_flatulans[k]->row() == m_player->row() && m_flatulans[k]->col() - 1== m_player->col());
        //Located Right
        bool Right = (m_flatulans[k]->row() == m_player->row() && m_flatulans[k]->col() + 1== m_player->col());

        if (Up || Down || Left || Right)
            m_player->getGassed();
    }
    //The player suffers a gas blast
}
...
{
    m_city->display();
    Player* p = m_city->player(); //returns a pointer to a player
    if (p == nullptr)
        return;
